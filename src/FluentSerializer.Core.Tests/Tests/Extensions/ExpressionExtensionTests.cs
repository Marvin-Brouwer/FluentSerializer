...
namespace FluentSerializer.Core.Tests.Tests.Extensions
{
	...
	public sealed class ExpressionExtensionTests
	{
		...
		public static IEnumerable<string[]> ValidCharacters()
		{
			...
			{
				yield return new string[] { char.ToLowerInvariant(letter).ToString() };
				yield return new string[] { letter.ToString() };
			}
			for (int number = 0; number <= 9; number++)
			{
				yield return new string[] { number.ToString() };
			}
			const string validSpecialCharacters = "_+-";
			foreach (var character in validSpecialCharacters)
			{
				yield return new string[] { character.ToString() };
			}
		}
